...
def weisfeiler_lehman_graph_hash(
    ...
    # Original algorithm implementation is copied from networkx package.
    ...
    for node in G.nodes():
        if (not node_attr) and (not edge_attr):
            node_labels[node] = str(G.degree(node)) # здесь нужно заменить: строка нам не очень подходит для перестановок
        elif node_attr:
            node_labels[node] = str(G.nodes[node][node_attr]) # и здесь
        else:
            node_labels[node] = "" # ну и здесь
